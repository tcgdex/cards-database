# Pokédex Data Remediation Workdir
...
```bash
bun run audit                # Quick summary
bun run audit:report         # Detailed report to file
```
...
```bash
bun run fix:dry-run          # Preview changes without modifying files
bun run fix:apply            # Apply changes to files
```
...
- `audit-report.txt` - Detailed audit results
- `pokemon-mapping.json` - Name to dexId mapping
- `fix-preview.txt` - Dry-run preview of changes
- `fix-log.txt` - Log of applied changes

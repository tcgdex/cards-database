...
async function main() {
	...
	for (const filePath of allFiles) {
		...
		try {
			...
			const resolvedDexIds = resolveDexId(engName, nameToId, manualMappings)
	...
	// Save detailed log
	const logPath = path.join(SCRIPT_DIR, dryRun ? 'check-missing-pokeid-dryrun.txt' : 'check-missing-pokeid-log.txt')

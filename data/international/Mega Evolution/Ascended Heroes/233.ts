import { Card } from "models/database/card"
import Set from "../Ascended Heroes"

const card: Card = {
	set: Set,

	name: {
		'en-us': "Misdreavus",
		'fr-fr': "Feuforêve",
		'es-es': "Misdreavus",
		'es-mx': "Misdreavus",
		'de-de': "Traunfugil",
		'it-it': "Misdreavus",
		'pt-br': "Misdreavus"
	},

	illustrator: "mashu",
	rarity: "Illustration rare",
	category: "Pokemon",
	dexId: [200],
	hp: 60,
	types: ["Psychic"],
	stage: "Basic",

	attacks: [{
		cost: ["Psychic"],

		name: {
			'en-us': "Ascension",
			'fr-fr': "Ascension",
			'es-es': "Ascensión",
			'es-mx': "Ascensión",
			'de-de': "Aufstieg",
			'it-it': "Ascensione",
			'pt-br': "Ascensão"
		},

		effect: {
			'en-us': "Search your deck for a card that evolves from this Pokémon and put it onto this Pokémon to evolve it. Then, shuffle your deck.",
			'fr-fr': "Cherchez dans votre deck une carte qui évolue de ce Pokémon, puis placez-la sur ce Pokémon pour le faire évoluer. Mélangez ensuite votre deck.",
			'es-es': "Busca en tu baraja 1 carta que evolucione de este Pokémon y ponla sobre este Pokémon para hacerlo evolucionar. Después, baraja las cartas de tu baraja.",
			'es-mx': "Busca en tu mazo 1 carta que evolucione de este Pokémon y ponla sobre este Pokémon para hacerlo evolucionar. Después, baraja tu mazo.",
			'de-de': "Durchsuche dein Deck nach 1 Karte, die sich aus diesem Pokémon entwickelt, und lege sie auf dieses Pokémon, um es zu entwickeln. Mische anschließend dein Deck.",
			'it-it': "Cerca nel tuo mazzo una carta che si evolve da questo Pokémon e mettila sopra questo Pokémon per farlo evolvere. Poi rimischia il tuo mazzo.",
			'pt-br': "Procure por uma carta no seu baralho que evolua deste Pokémon e coloque-a sobre este Pokémon para evoluí-lo. Em seguida, embaralhe o seu baralho."
		}
	}],

	weaknesses: [
		{
			type: "Darkness",
			value: "×2",
		},
	],
	resistances: [
		{
			type: "Fighting",
			value: "-30",
		},
	],
	retreat: 1,
	regulationMark: "I",

	description: {
		'en-us': "It loves to bite and yank people’s hair from behind without warning, just to see their shocked reactions.",
	},

	variants: [
		{
			type: "holo",
			thirdParty: {
				cardmarket: 869844,
				tcgplayer: 676045
			}
		}
	],
}

export default card

import { Card } from 'models/database/card'
import Set from '../Unleashed'

const card: Card = {
	name: {
		'en-us': "Sudowoodo",
		'fr-fr': "Simularbre",
		'de-de': "Mogelbaum"
	},

	illustrator: "Sachiko Adachi",
	rarity: "Holo Rare",
	category: "Pokemon",
	set: Set,

	dexId: [185],

	hp: 90,

	types: [
		"Fighting"
	],

	stage: "Basic",

	attacks: [
		{
			cost: [
				"Fighting",
			],
			name: {
				'en-us': "Push Over",
				'fr-fr': "Facilité",
				'de-de': "Umschubsen"
			},
			effect: {
				'en-us': "Does 20 damage times the amount of Fighting Energy attached to Sudowoodo.",
				'fr-fr': "Inflige 20 dégâts multipliés par le nombre de cartes Énergie Fighting attachées à Simularbre.",
				'de-de': "Dieser Angriff fügt 20 Schadenspunkte mal der Anzhal der an Mogelbaum angelegten -Energien zu."
			},
			damage: "20×",

		},
		{
			cost: [
				"Colorless",
				"Colorless",
				"Colorless",
			],
			name: {
				'en-us': "Rumble",
				'fr-fr': "Bagarre",
				'de-de': "Grollen"
			},
			effect: {
				'en-us': "The Defending Pokémon can't retreat during your opponent's next turn.",
				'fr-fr': "Le Pokémon Défenseur ne peut pas battre en retraite durant le prochain tour de votre adversaire.",
				'de-de': "Das Verteidigende Pokémon kann sich im nächsten Zug deines Gegners nicht zurückziehen."
			},
			damage: 40,

		},
	],

	weaknesses: [
		{
			type: "Water",
			value: "×2"
		},
	],

	retreat: 2,

	description: {
		'en-us': "It disguises itself as a tree to avoid attack. It hates water, so it will disappear if it starts raining."
	},

	variants: [
		{
			type: "holo",
			thirdParty: {
				tcgplayer: 89596,
				cardmarket: 279165
			}
		},
		{
			type: "reverse",
			thirdParty: {
				tcgplayer: 89596,
				cardmarket: 279165
			}
		},
	],

}

export default card

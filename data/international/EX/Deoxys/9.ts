import { Card } from 'models/database/card'
import Set from '../Deoxys'

const card: Card = {
	name: {
		'en-us': "Jirachi",
		'fr-fr': "Jirachi",
		'de-de': "Jirachi"
	},

	illustrator: "Sumiyoshi Kizuki",
	rarity: "Rare",
	category: "Pokemon",
	set: Set,

	dexId: [
		385,
	],

	hp: 60,

	types: [
		"Metal",
	],

	stage: "Basic",

	abilities: [
		{
			type: "Poke-POWER",
			name: {
				'en-us': "Wishing Star",
				'fr-fr': "Étoile à souhait",
				'de-de': "Wishing Star"
			},
			effect: {
				'en-us': "Once during your turn (before your attack), if Jirachi is your Active Pokémon, you may look at the top 5 cards of your deck, choose 1 of them, and put it into your hand. Shuffle your deck afterward. Jirachi and your other Active Pokémon, if any, are now Asleep. This power can't be used if Jirachi is affected by a Special Condition.",
				'fr-fr': "Une seule fois lors de votre tour (avant votre attaque), si Jirachi est votre Pokémon Actif, vous pouvez regarder les 5 cartes du dessus de votre deck, en choisir 1 et la placer dans votre main. Ensuite, mélangez votre deck. Jirachi et votre autre Pokémon Actif, si vous en avez un, sont maintenant Endormis. Ce pouvoir ne peut pas être utilisé si Jirachi est affecté par un État Spécial.",
				'de-de': "Once during your turn (before your attack), if Jirachi is your Active Pokémon, you may look at the top 5 cards of your deck, choose 1 of them and put it into your hand. Shuffle your deck afterward. Jirachi and your other Active Pokémon, if any, are now Asleep. This power can't be used if Jirachi is affected by a Special Condition."
			},
		},
	],

	attacks: [
		{
			cost: [
				"Metal",
				"Colorless",
			],
			name: {
				'en-us': "Metallic Blow",
				'fr-fr': "Coup métallique",
				'de-de': "Metallic Blow"
			},
			effect: {
				'en-us': "If the Defending Pokémon has any Poké-Bodies, this attack does 20 damage plus 30 more damage.",
				'fr-fr': "Si le Pokémon Défenseur possède des Poké-Bodies, cette attaque inflige 20 dégâts plus 30 dégâts supplémentaires.",
				'de-de': "If the Defending Pokémon has any Poke-Bodies, this attack does 20 damage plus 30 more damage."
			},
			damage: "20+",

		},
	],

	weaknesses: [
		{
			type: "Fire",
			value: "×2"
		},
	],

	resistances: [
		{
			type: "Grass",
			value: "-30"
		},
	],


	retreat: 1,


	variants: [
		{
			type: "reverse",
			stamp: ["set-logo"],
			thirdParty: {
				tcgplayer: 86327,
				cardmarket: 276412
			},
		},
		{
			type: "holo",
			thirdParty: {
				tcgplayer: 86327,
				cardmarket: 276412
			},
		},
		{
			type: "holo",
			stamp: ["curran-hill"],
			thirdParty: {
				tcgplayer: 477535,
				cardmarket: 871511
			},
		},
		{
			type: "holo",
			stamp: ["michael-gonzalez"],
			thirdParty: {
				tcgplayer: 477543,
				cardmarket: 871512
			},
		},
	],

}

export default card


import { Card } from "models/database/card";
import Set from "../S12a";

const card: Card = {
	set: Set,
	name: {
		'ja-jp': "ビーダル",
		'zh-tw': "大尾狸",
		'th-th': "บีดารุ",
	},

	illustrator: "Misa Tsutsui",
	category: "Pokemon",
	hp: 120,
	types: ["Colorless"],

	description: {
		'ja-jp': "川を 木の幹や 泥の ダムで せき止めて 住処を 作る。 働き者として 知られている。",
		'zh-tw': "用樹幹和泥巴築起水壩，堵住河流建造住所。 以勤勞而聞名。",
		'th-th': "จะสร้างที่อยู่อาศัยโดยใช้เขื่อนที่ทำมาจากกิ่งไม้หรือโคลนปิดกั้นแม่น้ำ เป็นที่รู้จักกันดีว่าเป็นคนขยัน",
	},

	stage: "Stage1",

	abilities: [
		{
			type: "Ability",
			name: {
				'ja-jp': "はたらくまえば",
				'zh-tw': "勤勞門牙",
				'th-th': "ฟันหน้าทำงาน",
			},
			effect: {
				'ja-jp': "自分の番に1回使える。自分の手札が5枚になるように、山札を引く。",
				'zh-tw': "在自己的回合時可使用1次。從牌庫抽卡直到自己的手牌滿5張為止。",
				'th-th': "ใช้ได้ 1 ครั้งในเทิร์นฝ่ายเรา จั่วการ์ดจากสำรับการ์ด จนได้การ์ดบนมือฝ่ายเราเป็น 5 ใบ",
			},
		},
	],

	attacks: [
		{
			name: {
				'ja-jp': "テールスマッシュ",
				'zh-tw': "長尾擊碎",
				'th-th': "เทลสแมช",
			},
			damage: 100,
			cost: ["Colorless", "Colorless", "Colorless"],
			effect: {
				'ja-jp': "コインを1回投げウラなら、このワザは失敗。",
				'zh-tw': "擲1次硬幣若為反面，則這個招式失敗。",
				'th-th': "ทอยเหรียญ 1 ครั้ง ถ้าออกก้อย ท่าต่อสู้นี้จะล้มเหลว",
			},
		},
	],

	weaknesses: [{ type: "Fighting", value: "x2" }],
	resistances: [],

	variants: [
		{
			type: "normal",
			thirdParty: {
				cardmarket: 687684,
				tcgplayer: 571660,
			},
		},
	],

	evolveFrom: {
		'ja-jp': "ビッパ",
	},

	retreat: 2,
	regulationMark: "F",
	rarity: "None",
	dexId: [400],
};

export default card;

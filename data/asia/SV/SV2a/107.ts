import { Card } from "models/database/card";
import Set from "../SV2a";

const card: Card = {
	set: Set,
	name: {
		'ja-jp': "エビワラー",
		'zh-tw': "快拳郎",
		'th-th': "เอบิวาลาร์",
		'id-id': "Hitmonchan",
	},

	illustrator: "DOM",
	category: "Pokemon",
	hp: 120,
	types: ["Fighting"],

	description: {
		'ja-jp': "空気をも 切り裂く パンチ。 だが ３分間 攻撃すると ひと休み したくなるらしい。",
		'zh-tw': "打出的拳擊甚至能劈開空氣。但連續攻擊３分鐘後， 牠似乎就會想休息一下。",
		'th-th': "หมัดฉีกอากาศ แต่ดูเหมือนว่าพอโจมตีไปได้ 3 นาทีก็ต้องการพัก",
		'id-id': "Pukulan Hitmonchan dapat membelah udara. Tapi dia butuh istirahat kalau sudah menyerang selama tiga menit.",
	},

	stage: "Basic",

	abilities: [
		{
			type: "Ability",
			name: {
				'ja-jp': "はんげき",
				'zh-tw': "反擊",
				'th-th': "ตีตอบโต้",
				'id-id': "Serangan Balasan",
			},
			effect: {
				'ja-jp': "このポケモンが、バトル場で相手のポケモンからワザのダメージを受けたとき、ワザを使ったポケモンにダメカンを3個のせる。",
				'zh-tw': "這隻寶可夢在戰鬥場受到對手的寶可夢招式的傷害時，在使用招式的寶可夢身上放置3個傷害指示物。",
				'th-th': "เมื่อโปเกมอนนี้ อยู่บนตำแหน่งต่อสู้และได้รับแดเมจของท่าต่อสู้จากโปเกมอนฝ่ายตรงข้าม วางตัวนับแดเมจ 3 ตัวบนโปเกมอนที่ใช้ท่าต่อสู้",
				'id-id': "Saat Pokémon ini ada di Arena Bertarung dan menerima kerusakan akibat serangan dari Pokémon lawan, letakkan 3 Token Kerusakan pada Pokémon yang telah menggunakan serangan.",
			},
		},
	],

	attacks: [
		{
			name: {
				'ja-jp': "エキサイトパンチ",
				'zh-tw': "激動拳",
				'th-th': "เอ็กซ์ไซต์พันช์",
				'id-id': "Excite Punch",
			},
			damage: 60,
			cost: ["Fighting", "Fighting"],
			effect: {
				'ja-jp': "次の自分の番、このポケモンの「エキサイトパンチ」のダメージは「+60」される。",
				'zh-tw': "在下個自己的回合，這隻寶可夢「激動拳」的傷害「+60」點。",
				'th-th': "เทิร์นถัดไปของฝ่ายเรา แดเมจจาก [เอ็กซ์ไซต์พันช์] ของโปเกมอนนี้จะถูก [+60]",
				'id-id': "Pada giliran sendiri berikutnya, kerusakan akibat Excite Punch Pokémon ini bertambah sejumlah 60.",
			},
		},
	],

	weaknesses: [{ type: "Psychic", value: "x2" }],
	resistances: [],

	variants: [
		{
			type: "normal",
			thirdParty: {
				cardmarket: 719560,
			},
		},
		{
			type: "reverse",
			foil: "pokeball",
			thirdParty: {
				cardmarket: 837442,
			},
		},
		{
			type: "reverse",
			foil: "masterball",
			thirdParty: {
				cardmarket: 837443,
			},
		},
	],

	retreat: 1,
	regulationMark: "G",
	rarity: "Uncommon",
	dexId: [107],
};

export default card;

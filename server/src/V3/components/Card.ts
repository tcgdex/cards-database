...
export async function getAllCards(lang: SupportedLanguages): Promise<Array<Card>> {
	return Promise.all((cards[lang] as Array<MappedCard>).map((it) => loadCard(lang, it.id))) as Promise<Array<Card>>
}
 ...
 * @param id
 */
async function loadCard(lang: SupportedLanguages, id: string): Promise<Card | null> {
	...
	const d_set = await getSetById(lang,card.set.id)

...
describe('buildSetNumber', () => {
	...
	it('uses subset denominator for prefixed local ids when subset metadata is available', () => {
		const card = {
			name: { en: 'Trainer Gallery' },
			category: 'Trainer',
			rarity: 'Rare',
			set: buildSet('swsh9', 172, {
				TG: {
					name: { en: 'Trainer Gallery' },
		...
		const result = buildSetNumber('TG09', card)
		expect(result).toEqual({
			text: 'TG09/TG30',
			nominator: 'TG09',
			numeric: 9,
	...
	it('falls back to parent set denominator for prefixed local ids when subset metadata is missing', () => {
		const card = {
			name: { en: 'Trainer Gallery' },
			category: 'Trainer',
			rarity: 'Rare',
